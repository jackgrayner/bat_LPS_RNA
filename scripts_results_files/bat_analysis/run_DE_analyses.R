...
# nb. script assumes you have run load_data_functions_summarise.R
...
## plot PCA
pca.trt<-plotPCA(vst(dds.p7),intgroup=c("Sex","Est.Age","Band","Phase.batch","Trtmt"),returnData=TRUE,ntop=1000)
pca.trt$Trtmt<-factor(pca.trt$Trtmt)
levels(pca.trt$Trtmt)=c("none","LPS")
p7.pca<-ggplot(pca.trt,aes(x=PC1,y=PC2,fill=Trtmt))+
  cust.theme()+
  geom_line(aes(group=Band),colour="#aaaaaa",linewidth = 0.25)+
  geom_point(size=3,shape=21)+scale_fill_manual(values=TrtPalette)+
  xlab("PC1 (51% var.)")+ylab("PC2 (8% var.)")+
  theme(legend.position = "inside",legend.title=element_blank(),
        legend.position.inside = c(0.9, 0.1),
        legend.box.background = element_rect(colour = "#999",fill="#fffff9", linewidth = 0.1))+
  guides(fill = guide_legend(override.aes = list(size = 2)))+
  coord_cartesian(xlim=c(-45,45),ylim=c(-30,22))

## plot differences in PC1 between untreated and LPS treated samples
pca.paired<-merge(pca.trt[pca.trt$Trtmt=="none",],pca.trt[pca.trt$Trtmt=="LPS",],by="Band")
pca.paired$diff<-pca.paired$PC1.y-pca.paired$PC1.x
p7.pcdiff<-ggplot(pca.paired,aes(x=Est.Age.x,y=diff,fill=Sex.x))+
  cust.theme()+geom_smooth(method='lm',aes(colour=Sex.x))+
  geom_point(shape=21,size=2,stroke=0.25,alpha=0.8)+
  scale_colour_manual(values=SexPalette)+
  scale_fill_manual(values=SexPalette)+
  labs(fill="Sex",colour='Sex',x="Age",y="PC1 difference")

#ggsave("paired_PC1_diff_sexage.png",dpi=600,height=3.5,width=4.5)

## run LM of PC1, testing sex and age effects
lm.pca.paired<-lmer(diff~Sex.x*Est.Age.x+(1|Phase.batch.x),data=pca.paired)
plot(lm.pca.paired)
qqnorm(resid(lm.pca.paired))
qqline(resid(lm.pca.paired))
car::Anova(lm.pca.paired,type="III")#sex * age interaction

### Plot Volcano  for TH-affected genes
g.volcano<-plot_volcano(res.th.vs.c0)+coord_cartesian(ylim=c(0,310))
...
kk2 <- gseKEGG(geneList=kegg_gene_list,organism='human',
...
## create vector of LPS GO term genes sig. in at least one spp. 
...
g.cor.mac<-ggplot(imp.gene.lfc,aes(x=bat,y=macaque))+cust.theme()+xlab("bat Log2FC")+ylab("macaque Log2FC")+
...
write.csv(dds.trt.c0.age,"./phases3-7_C0_age_effect.csv",row.names = FALSE)
write.csv(dds.trt.c0.sex,"./phases3-7_C0_sex_effect.csv",row.names = FALSE)


#####################
# 07. VARIATION IN LPS-TREATED SAMPLES - BOTH SEXES

samples.all.th$MonYear<-paste(samples.all.th$Mon,samples.all.th$Yr,sep="_")
...
#summary((results(dds.all.th, name="Cave_Tamana_vs_IH2",alpha=alpha)))
...
dds.trt.th.sex$LPS_affected<-FALSE
dds.trt.th.sex[dds.trt.th.sex$gene %in% lps.genes,]$LPS_affected<-TRUE
table(dds.trt.th.sex$sig,dds.trt.th.sex$LPS_affected)
fisher.test(table(dds.trt.th.sex$sig,dds.trt.th.sex$LPS_affected))

#are age-associated genes overrepresented for LPS-affected genes
dds.trt.th.age<-dds.trt.th.age[!is.na(dds.trt.th.age$padj),]
dds.trt.th.age$LPS_affected<-FALSE
dds.trt.th.age[dds.trt.th.age$gene %in% lps.genes,]$LPS_affected<-TRUE
table(dds.trt.th.age$sig,dds.trt.th.age$LPS_affected)
...
g.c0.th.sexcomp<-ggplot(dds.sex.c0.th.thsig,aes(x=log2FoldChange.x,y=log2FoldChange.y,fill=log2FoldChange.x*log2FoldChange.y>0))+
  cust.theme()+
  geom_hline(yintercept = 0,linetype='dashed',linewidth=0.25)+
  geom_vline(xintercept = 0,linetype='dashed',linewidth=0.25)+
  geom_abline(intercept=0,slope=1,linetype='solid',linewidth=0.25)+
  geom_point(shape=21,alpha=0.75,stroke=0.25)+#geom_smooth(method='lm')+
...
g.c0.th.agecomp<-ggplot(dds.age.c0.th.thsig,aes(x=log2FoldChange.x,y=log2FoldChange.y,fill=log2FoldChange.x*log2FoldChange.y>0))+
  cust.theme()+
  geom_hline(yintercept = 0,linetype='dashed',linewidth=0.25)+
  geom_vline(xintercept = 0,linetype='dashed',linewidth=0.25)+
  geom_abline(intercept=0,slope=1,linetype='solid',linewidth=0.25)+
...
age.vol<-plot_volcano(dds.trt.th.age)+
  labs(tag="D",subtitle='Age-associated (LPS-treated)',x='log2FC')

## go plots for sex
go.th.sex.m<-plot_GO_overrep(dds.trt.th.sex,'up')+ labs(subtitle="Sex: M up (log2FC > 0)",tag="C")
go.th.sex.f<-plot_GO_overrep(dds.trt.th.sex,'dn')+ labs(subtitle="Sex: F up (log2FC < 0)")

## go plots for age
go.th.age.up<-plot_GO_overrep(dds.trt.th.age,'up')+ labs(subtitle="Age: up (log2FC > 0)",tag="F")
go.th.age.dn<-plot_GO_overrep(dds.trt.th.age,'dn')+ labs(subtitle="Age: down (lo2gFC < 0)")
...
table(dds.th.age$log2FoldChange.x*dds.th.age$log2FoldChange.y>0,
      dds.th.age$sig.x | dds.th.age$sig.y)
fisher.test(table(dds.th.age$log2FoldChange.x*dds.th.age$log2FoldChange.y>0,
      dds.th.age$sig.x | dds.th.age$sig.y))
table(dds.th.age$log2FoldChange.x*dds.th.age$log2FoldChange.y>0,
      dds.th.age$sig.x & dds.th.age$sig.y)
...
#age distribution panels
g.age<-ggplot(samples.all.th,aes(x=Age,fill=Sex))+theme_minimal()+geom_density(alpha=0.5,linewidth=0.25)+
...
g.ss.age<-ggplot(samples.all.th,aes(x=ss.age,fill=Sex))+theme_minimal()+geom_density(alpha=0.5,linewidth=0.25)+
...
g.slope.th<-ggplot(dds.th.age,aes(x=log2FoldChange.x,y=log2FoldChange.y,colour=sig))+
  ...
  geom_point(data=dds.th.age[dds.th.age$sig.x | dds.th.age$sig.y,],
             alpha=0.75,shape=21,stroke=0.25,aes(fill=sig),size=2,colour='black')+scale_fill_manual(values=c(SexPalette,"#b459ff"))+
  geom_point(data=dds.th.age[dds.th.age$sig.x & dds.th.age$sig.y,],#overlay genes sig in both sexes, so they're on top layer
             alpha=0.75,shape=21,stroke=0.25,size=2,colour='black',aes(x=log2FoldChange.x,y=log2FoldChange.y),fill='#b459ff')+
...
g.age+labs(tag="G")+g.ss.age+labs(tag="H")+
  g.slope.th+theme(legend.position='none')+
  coord_cartesian(xlim=c(-3.5,2.5),ylim=c(-3.5,2.5))+
  g.ss.slope.th+labs(fill="Padj. < 0.05")+coord_cartesian(xlim=c(-2.5,2),ylim=c(-2.5,2))+
  plot_layout(heights=c(0.2,1))

ggsave('sex_age.svg',width=8,height=4.25)
#ggsave('sex_age_nodupes.svg',width=8,height=4.25)
